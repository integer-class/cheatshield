...
class FaceAnalyzer:
    ...
    def detect_face(self, image):
        """
        Detect single largest face in image
    ...
    def analyze_yaw(self, image, face):
        """
        Calculate face yaw (horizontal angle)
        
        Args:
            image: RGB image array
            face: tuple of (x, y, w, h)
        Returns:
            float: Absolute yaw angle in degrees
        """
        x, y, w, h = face
        ...
        yaw_angle = abs((relative_x - 0.5) * 90)  # Convert to degrees
        
        return yaw_angle
    
    def analyze_pitch(self, image, face):
        """
        Calculate face pitch (vertical angle)
        
        Args:
            image: RGB image array
            face: tuple of (x, y, w, h)
        Returns:
            float: Absolute pitch angle in degrees
        """
        x, y, w, h = face
    ...
    def analyze_roll(self, image, face):
        """
        ...
        Args:
            ...
            face: tuple of (x, y, w, h)
        Returns:
            float: Absolute roll angle in degrees
        """
        x, y, w, h = face
        ...
        roll_angle = abs(np.arctan2(np.mean(gradient_y), np.mean(gradient_x)) * 180 / np.pi)
        
        return roll_angle
    
    def analyze_blur(self, image, face):
        """
        Calculate face blur level using multiple methods
        
        Args:
            image: RGB image array
            face: tuple of (x, y, w, h)
        Returns:
            float: Blur score (higher means less blur)
        """
        x, y, w, h = face
        face_roi = image[y:y+h, x:x+w]
        gray_roi = cv2.cvtColor(face_roi, cv2.COLOR_BGR2GRAY)
        ...
        fft_score = magnitude_spectrum[crow-5:crow+5, ccol-5:ccol+5].mean()
...
def sort_and_select_best_frames(frames_directory, sorted_directory, top_n=5):
    """
    Sort frames by face quality metrics and export best frames
    """
    ...
    def calculate_frame_score(frame_info):
